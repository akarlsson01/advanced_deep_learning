"""
Python code for the GAN: generation of MNIST digits assignment.

Author: Albin Karlsson
From template by course teachers.
"""
...
class Generator(nn.Module):
    ...
    def __init__(self):
        ...
        self.gen = nn.Sequential(
            nn.Linear(
                latent_dimension, 2*latent_dimension
            ),  # TODO define a suitable hidden dimension
            # TODO: define a suitable network architecture for the generator
            nn.LeakyReLU(0.2),  # It is helpful to use the tanh activation function to force the ouput into the [-1,1] range that our normalized images have.
            nn.Linear(2 * latent_dimension, 4 * latent_dimension),
            nn.LeakyReLU(0.2),
            nn.Linear(4 * latent_dimension, image_dimension),
            nn.Tanh(),
        )
...
for epoch in range(numEpochs):
    ...
    for batch_idx, (real, _) in enumerate(loader):
        ...
        prediction = discriminator(real)
        loss_discriminator_real = criterion(prediction, true_labels)
        ...
        loss_discriminator_fake = criterion(prediction, false_labels)
        ...
        # We start by generating new fake images
        ...
        loss_generator = criterion(prediction, true_labels)
        ...
        # print the progress
        print(
            f"\rEpoch [{epoch+1}/{numEpochs}] Batch {batch_idx}/{len(loader)} \ Loss discriminator: {loss_discriminator:.4f}, loss generator: {loss_generator:.4f}",
            end="",
        )
        ...
        if batch_idx % logStep == 0:
            with torch.no_grad():
                ...
                writer.add_scalar("Discriminator loss", loss_discriminator.item(), step)
                writer.add_scalar("Generator loss", loss_generator.item(), step)
